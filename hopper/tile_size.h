...
#include <tuple>
  ...
  // Custom tilesize selection function
constexpr std::tuple<int, int, bool, bool> custom_tile_size_fwd_sm90_fp16(
    ...
    if (headdim <= 64) {
        return {192, is_causal || is_local ? 128 : 128, true, true};
    } else if (headdim <= 96) {
        return {192, is_local || paged_kv_non_TMA ? 96 : 128, false, true};
    } else if (headdim <= 128) {
        return {128, is_causal || is_local || paged_kv_non_TMA ? 96 : 128, true, true};
    } else if (headdim <= 192) {
        return {128, paged_kv_non_TMA || is_local ? 64 : 96, true, true};
    } else {
        return {128, is_local ? 48 : 64, true, false};
    }
}

// Return {kBlockM, kBlockN, MmaPV_is_RS, IntraWGOverlap}
constexpr std::tuple<int, int, bool, bool> custom_tile_size_fwd_sm90_fp8(
    int headdim, int headdim_v, bool is_causal, bool is_local, int element_size=2,
    bool v_colmajor=false, bool paged_kv_non_TMA=false, bool softcap=false
){
    if (headdim <= 64) {
        // was: {192, 160, true, true};
        // the below config however yields much better performance
        return {192, 128, true, true};
    } else if (headdim <= 96) {
        return {192, 128, true, true};
    } else if (headdim <= 128) {
        // was: return {128, paged_kv_non_TMA ? 160 : (v_colmajor || (softcap && is_local) ? 192 : 224), true, true};
        // this also yields good performance
        return {128, paged_kv_non_TMA ? 160 : (v_colmajor || (softcap && is_local) ? 192 : 192), true, true};
    } else if (headdim <= 192) {
...
constexpr std::tuple<int, int, bool, bool> tile_size_fwd_sm90(
    ...
    if (element_size == 2) {
        #if USE_MMA_SOFTMAX  // add <xie>
        return custom_tile_size_fwd_sm90_fp16(
            headdim, headdim_v, is_causal, is_local,
        ...
        #if USE_MMA_SOFTMAX
        return custom_tile_size_fwd_sm90_fp8(
